...
const SUPABASE_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InhscGR2c29kZ3JkbHB3dWFqenlyIiwicm9sZSI6ImFub24iLCJpYXQiOjE3Nzc5MjkyMDcsImV4cCI6MjA5MzUwNTIwN30.RjVzdzv3daq84cTvUdwguFpNpSwXKPNL2iJf7KOLT7Q';
...
function slideOut(el, direction = 'left') {
  return new Promise(resolve => {
    ...
    const cls = direction === 'left' ? 'anim-slide-out-left' : 'anim-slide-out-right';
...
function slideIn(el, direction = 'right') {
  if (!el) return;
  const cls = direction === 'right' ? 'anim-slide-in-right' : 'anim-slide-in-left';
...
function startStripeCheckout() {
  ...
  const url = email ? `${STRIPE_PAYMENT_LINK}?prefilled_email=${encodeURIComponent(email)}` : STRIPE_PAYMENT_LINK;
...
function isPremium() { return localStorage.getItem('nour_premium') === 'true'; }

function showPremiumBadge() {
  if (isPremium()) document.getElementById('premiumBadge').classList.remove('hidden');
}
...
function setupEmailEvents() {
  ...
  submitBtn.onclick = async () => {
    ...
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
      input.classList.add('error'); errorEl.classList.remove('hidden'); return;
    }
    input.classList.remove('error'); errorEl.classList.add('hidden');
    submitBtn.textContent = 'Sauvegarde...'; submitBtn.disabled = true;
    ...
    submitBtn.textContent = 'Sauvegarder et continuer'; submitBtn.disabled = false;
  ...
  input.addEventListener('input', () => { input.classList.remove('error'); errorEl.classList.add('hidden'); });
}
...
function loadProfiles() { return JSON.parse(localStorage.getItem('nour_profiles') || '[]'); }
function saveProfiles(profiles) { localStorage.setItem('nour_profiles', JSON.stringify(profiles)); }
...
function selectProfile(index) {
  ...
  const defaultState = {
    currentWordIndex: 0, learnedWords: [], streak: 0, xp: 0,
    lastVisit: null, totalQuizzes: 0, correctQuizzes: 0,
    quizActive: false, mode: 'learn', srsData: {}
  };
  state = profiles[index].state ? { ...defaultState, ...profiles[index].state } : defaultState;
  if (!state.srsData) state.srsData = {};
...
function deleteProfile(e, index) {
  ...
  if (confirm(`Supprimer "${profiles[index].name}" ?`)) {
    profiles.splice(index, 1); saveProfiles(profiles); renderProfilesGrid();
  }
}
...
function setupProfileEvents() {
  ...
  addBtn.onclick = () => {
    if (loadProfiles().length >= 4) { showToast('Maximum 4 profils'); return; }
  ...
  document.getElementById('cancelProfileBtn').onclick = () => modal.classList.add('hidden');

  document.getElementById('avatarPicker').querySelectorAll('.avatar-opt').forEach(opt => {
...
function getKidMessage(type) {
  const messages = {
    correct: ['🌟 Super !', '🎉 Bravo !', '✨ Génial !', '🏆 Excellent !', '🌈 Parfait !'],
    wrong: ['😊 Presque !', '💪 Réessaie !', '🤔 Pas encore...', '😅 Continue !'],
...
function nextSlide() {
  ...
  if (currentSlide === slides.length - 1) document.getElementById('onboardingNext').textContent = 'Continuer →';
}
...
function renderLearnScreen(direction = 'right') {
  ...
  document.getElementById('lessonBadge').textContent = `Mot ${state.currentWordIndex + 1} / ${WORDS.length}`;
  ...
  document.getElementById('nextWord').textContent = (!isPremium() && nextIndex >= FREE_WORDS_LIMIT) ? '🔒 Débloquer' : "J'ai compris →";
...
function renderQuizScreen() {
  ...
  slideOut(card, 'left').then(() => {
    ...
    document.getElementById('quizQuestion').textContent = 'Quel est le sens de ce mot ?';
    ...
    const wrongs = WRONG_ANSWERS.filter(w => !correct.includes(w)).sort(() => Math.random() - 0.5).slice(0, 3);
    ...
    options.forEach((opt, i) => {
      ...
      btn.addEventListener('click', () => handleLearnQuizAnswer(btn, opt, correct, word.id));
  ...
  if (state.mode === 'quiz') {
    ...
    if (!state.learnedWords.includes(wordId)) {
      ...
      if (milestones.includes(count)) {
        ...
        setTimeout(() => showShareTrigger('milestone', {
          count, levelName: level.name, levelIcon: level.icon
        }), 1500);
      }
    }
...
function goPrevWord() {
  if (state.mode === 'quiz') { renderLearnScreen('left'); return; }
...
function handleLearnQuizAnswer(btn, selected, correct, wordId) {
  state.totalQuizzes++;
  document.querySelectorAll('#quizSection .quiz-option').forEach(b => b.style.pointerEvents = 'none');
  ...
  if (isCorrect) {
    ...
    haptic([50]);
    setTimeout(() => { if (state.mode === 'quiz') goNextWord(); }, isKidProfile() ? 1800 : 1400);
  } else {
    btn.classList.add('wrong');
    document.querySelectorAll('#quizSection .quiz-option').forEach(b => {
      if (b.textContent === correct) b.classList.add('correct');
    });
    const errMsg = isKidProfile() ? `${getKidMessage('wrong')} C'était : ${correct}` : `❌ C'était : ${correct}`;
...
function startSRSSession() {
  const toReview = getWordsToReviewToday();
  srsQueue = toReview.map(id => WORDS.find(w => w.id === id)).filter(Boolean).sort(() => Math.random() - 0.5);
  srsCurrentIndex = 0;
  renderSRSCard();
}

function renderSRSCard() {
  if (srsCurrentIndex >= srsQueue.length) { renderSRSComplete(); return; }
...
function handleSRSAnswer(btn, selected, correct, wordId) {
  document.querySelectorAll('.srs-options .quiz-option').forEach(b => b.style.pointerEvents = 'none');
  ...
  if (isCorrect) {
    ...
    setTimeout(() => {
      srsCurrentIndex++;
      saveState(); updateUI(); updateReviewBadge();
      renderSRSCard();
    }, 1200);
  } else {
    btn.classList.add('wrong');
    document.querySelectorAll('.srs-options .quiz-option').forEach(b => { if (b.textContent === correct) b.classList.add('correct'); });
    ...
    cont.onclick = () => { srsCurrentIndex++; saveState(); updateUI(); updateReviewBadge(); renderSRSCard(); };
...
function renderReviewGrid() {
  ...
  html += state.learnedWords.map(id => {
...
function updateReviewBadge() {
  ...
  if (badge) {
    if (toReview.length > 0) { badge.textContent = toReview.length; badge.classList.remove('hidden'); }
...
function renderProgress() {
  ...
  document.getElementById('statQuiz').textContent = state.totalQuizzes > 0 ? Math.round(state.correctQuizzes / state.totalQuizzes * 100) + '%' : '0%';
  const level = LEVELS.find(l => state.learnedWords.length >= l.min && state.learnedWords.length <= l.max) || LEVELS[LEVELS.length - 1];
...
function checkBadges() { updateBadges(); }
...
function updateStreak() {
  ...
  if (!state.lastVisit) { state.streak = 1; }
    ...
    state.streak = state.lastVisit === yesterday.toDateString() ? state.streak + 1 : 1;
...
function updateUI() {
  ...
  document.getElementById('progressLabel').textContent = `${state.learnedWords.length} / 200 mots`;
}
...
function playAudio() {
  ...
  if (window.currentAudio) {
    ...
    btn.innerHTML = '🔊 Écouter le verset'; btn.style.opacity = '1'; return;
  }
  btn.innerHTML = '⏳ Chargement...'; btn.style.opacity = '0.7';
  const audio = new Audio(`https://cdn.islamic.network/quran/audio/128/ar.alafasy/${word.verseNumber || 1}.mp3`);
  window.currentAudio = audio;
  audio.oncanplay = () => { btn.innerHTML = '⏸ Pause'; btn.style.opacity = '1'; audio.play(); };
  audio.onended = () => { btn.innerHTML = '🔊 Écouter le verset'; btn.style.opacity = '1'; window.currentAudio = null; };
  audio.onerror = () => { btn.innerHTML = '🔊 Écouter le verset'; btn.style.opacity = '1'; window.currentAudio = null; showToast('Audio non disponible'); };
}
